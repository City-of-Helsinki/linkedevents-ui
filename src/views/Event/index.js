...
class EventPage extends React.Component {
    ...
    render() {
        ...
        if (event.publication_status === constants.PUBLICATION_STATUS.DRAFT) {
            draftBadge = (<span style={{marginRight:'0.5em'}} className="badge badge-warning text-uppercase"><FormattedMessage id="draft"/></span>)
        }
        ...
        if (event.event_status === constants.EVENT_STATUS.CANCELLED) {
            cancelledBadge = (<span style={{marginRight:'0.5em'}} className="badge badge-danger text-uppercase"><FormattedMessage id="cancelled"/></span>)
        }
