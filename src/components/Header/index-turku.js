...
import cityOfTurkuLogo from '../../assets/images/turkulogowhite.png';
import {hasOrganizationWithRegularUsers} from '../../utils/user';
import {get} from 'lodash';
import {TkuMaterialTheme} from '../../themes/material-ui-tku';
import {
    TkuSelectTheme,
    TkuLanguageSelectStyles,
} from '../../themes/react-select-tku';
import moment from 'moment';
import * as momentTimezone from 'moment-timezone';

const {
    USER_TYPE,
    APPLICATION_SUPPORT_TRANSLATION,
} = constants;
    ...
    componentDidMount() {
        ...
        if (user) {
            const showModerationLink =
                get(user, 'userType') === USER_TYPE.ADMIN &&
    ...
    render() {
        ...
        return (
            <div className='main-navbar'>
                <Toolbar className='turku-bar'>
                    <div className='turku-bar__logo'>
                        <Link to='/'>
                            <img
                                src={cityOfTurkuLogo}
                                alt='City Of Turku'
                    ...
                    <div className='turku-bar__login-button'>
                        <div className='turku-bar__language-button'>
                                ...
                                <Drawer
                                    ...
                                    <div className='menu-drawer-mobile'>
                                        <NavLinks
                                            ...
                                            toModerationPage={this.getNavigateMobile(
                                                toModerationPage,
                                            )}
                                        />
                                    </div>
    ...
    return (
        <React.Fragment>
            <Button onClick={toMainPage}>
                <FormattedMessage
                    id={`${appSettings.ui_mode}-management`}
                />
            </Button>
            <Button onClick={toSearchPage}>
                <FormattedMessage
                    id={`search-${appSettings.ui_mode}`}
                />
            </Button>
